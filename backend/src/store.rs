...
use crate::model::{
	AdminQuery, AutomationMode, Board, BoardPatch, BoardStatus, Comment, CreateComment,
	CreateGuestRequest, MergeResult, PublicBoard, PublicComment, PublicQuery, PublicRequest,
	AutomationRun, PublicStatus, Release, Request, RequestPatch, Timestamp, VoteResult, Workspace,
	WorkspacePatch,
};
...
impl Store {
	...
	pub fn create_board(&self, raw_slug: &str, raw_name: &str, raw_description: &str) -> Result<Board> {
		...
		drop(guard);
		self.board_by_id(&id)?.ok_or_else(|| StoreError::NotFound.into())
	}
	...
	pub fn list_public_requests(
		...
		let search = query.query.as_deref().map(str::trim).filter(|value| !value.is_empty());
	...
	pub fn create_release(&self, title: &str, body: &str, request_ids: &[String]) -> Result<Release> {
	...
	pub fn publish_release(&self, id: &str, body: Option<&str>) -> Result<Release> {
		...
		drop(guard);
		self.public_release(id)?.ok_or_else(|| StoreError::NotFound.into())
	}
	...
	pub fn create_guest_request(
		...
		if input.honeypot.as_deref().unwrap_or_default().trim().is_empty().not() {
		...
		self.insert_request(
			...
			&[],
			0,
			if board.moderate_public_writes { "pending" } else { "approved" },
	...
	pub fn add_public_comment(
		...
		if self
			.public_request(slug, request_id)?
			.is_none()
		{
			return Err(StoreError::PublicRequestUnavailable.into());
		}
	...
	pub fn list_admin_requests(&self, query: &AdminQuery) -> Result<Vec<Request>> {
		...
		sql.push_str(&format!(" ORDER BY updated_at DESC LIMIT ?{next_parameter}"));
		values.push(Value::Integer(limit));
		let mut statement = guard.prepare(&sql)?;
		let mut rows = statement.query(rusqlite::params_from_iter(values.iter()))?;
		let mut output = Vec::new();
	...
	pub fn patch_request(&self, id: &str, expected_revision: i64, patch: RequestPatch) -> Result<Request> {
		...
		if let Some(value) = patch.status {
			if !self.statuses(&next.board_id)?.iter().any(|status| status.code == value) {
		...
		drop(guard);
		self.request_by_id(id)?.ok_or_else(|| StoreError::NotFound.into())
	}
	...
	pub fn merge_requests(&self, survivor_id: &str, duplicate_id: &str) -> Result<MergeResult> {
		...
		let survivor = self.request_by_id(survivor_id)?.ok_or(StoreError::NotFound)?;
		let duplicate = self.request_by_id(duplicate_id)?.ok_or(StoreError::NotFound)?;
		...
		drop(guard);
		Ok(MergeResult {
			survivor: self.request_by_id(survivor_id)?.ok_or(StoreError::NotFound)?,
	...
	pub fn create_automation_run(
		...
		drop(guard);
		self.automation_run(&id)?.ok_or_else(|| StoreError::NotFound.into())
	}
	...
	pub fn patch_workspace(&self, expected_revision: i64, patch: WorkspacePatch) -> Result<Workspace> {
		let current = self.workspace()?;
		if current.revision != expected_revision {
			return Err(StoreError::StaleRevision.into());
		}
		let next_name = patch.name.map(|value| value.trim().to_owned()).unwrap_or(current.name);
		let next_description = patch.description.map(|value| value.trim().to_owned()).unwrap_or(current.description);
		let next_color = patch.primary_color.map(|value| value.trim().to_owned()).unwrap_or(current.primary_color);
		let next_mode = patch.default_automation_mode.unwrap_or(current.default_automation_mode);
		let next_agent = patch.default_agent_id.unwrap_or(current.default_agent_id);
		let next_workflow = patch.default_workflow_id.unwrap_or(current.default_workflow_id);
		let next_guest = patch.allow_guest_posts.unwrap_or(current.allow_guest_posts);
		let next_moderate = patch.moderate_public_writes.unwrap_or(current.moderate_public_writes);
		let guard = self.conn.lock().expect("feedback board store poisoned");
		let updated = guard.execute(
			...
			params![DEFAULT_WORKSPACE_ID, next_name, next_description, next_color, next_mode.as_str(), next_agent, next_workflow, next_guest, next_moderate, current.revision + 1, now_ms(), expected_revision],
		)?;
		if updated == 0 {
			return Err(StoreError::StaleRevision.into());
		}
	...
	pub fn patch_board(&self, id: &str, expected_revision: i64, patch: BoardPatch) -> Result<Board> {
		...
		let updated = guard.execute(
			...
			params![id, next_name, next_description, next_mode.map(AutomationMode::as_str), next_guest, next_moderate, current.revision + 1, now_ms(), expected_revision],
		)?;
		if updated == 0 {
			return Err(StoreError::StaleRevision.into());
		}
	...
	fn insert_request(
		...
		guard.execute(
			...
			params![id, board_id, title, body, author_name, author_email, category, status, serde_json::to_string(tags)?, vote_count, moderation_state, public_visible, now],
		)?;
		Ok(())
	}
}
...
fn read_workspace(row: &Row<'_>) -> rusqlite::Result<Workspace> {
	Ok(Workspace {
		...
		default_automation_mode: AutomationMode::try_from(row.get::<_, String>(5)?.as_str())
			.map_err(|error| rusqlite::Error::FromSqlConversionFailure(5, rusqlite::types::Type::Text, Box::new(std::io::Error::other(error))))?,
...
fn read_request(row: &Row<'_>) -> rusqlite::Result<Request> {
	...
	Ok(Request {
		...
		automation_mode: mode
			...
			.map(AutomationMode::try_from)
			.transpose()
			.map_err(|error| rusqlite::Error::FromSqlConversionFailure(22, rusqlite::types::Type::Text, Box::new(std::io::Error::other(error))))?,
		revision: row.get(23)?,
...
fn read_automation_run(row: &Row<'_>) -> rusqlite::Result<AutomationRun> {
	let mode = AutomationMode::try_from(row.get::<_, String>(2)?.as_str())
		.map_err(|error| rusqlite::Error::FromSqlConversionFailure(2, rusqlite::types::Type::Text, Box::new(std::io::Error::other(error))))?;
	Ok(AutomationRun {
		id: row.get(0)?,
		request_id: row.get(1)?,
		mode,
