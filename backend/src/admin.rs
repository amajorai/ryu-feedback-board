...
pub fn routes(ctx: Arc<Ctx>) -> Router {
    Router::new()
        ...
        .route("/requests/:requestId", get(get_request).put(patch_request))
...
async fn list_requests(
    ...
    Ok(Json(
        json!({ "requests": ctx.store.list_admin_requests(&query)? }),
    ))
}
...
async fn apply_triage(
    ...
    Ok(Json(
        json!({ "request": request, "suggestion": input.suggestion }),
    ))
}
...
async fn merge_request(
    ...
    let result = ctx
        .store
        .merge_requests(&request_id, &input.duplicate_request_id)?;
