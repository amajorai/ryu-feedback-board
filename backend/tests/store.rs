use ryu_feedback_board::model::{CreateGuestRequest, PublicRequest, PublicStatus, Request, RequestPatch};
...
fn merge_moves_public_activity_to_the_survivor() {
	...
	assert!(store.public_request("feedback", &second.id).unwrap().is_none());
}
